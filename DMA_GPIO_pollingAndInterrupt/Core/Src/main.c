...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
uint16_t led_state[2] = {0x00, 0x4000};
...
int main(void)
{
  ...
  {
    /* USER CODE END WHILE */
	  HAL_DMA_Start(&hdma_memtomem_dma2_channel1, (uint32_t)&led_state[0], (uint32_t)&GPIOB->ODR, 1);
	  HAL_DMA_PollForTransfer(&hdma_memtomem_dma2_channel1, HAL_DMA_FULL_TRANSFER, HAL_MAX_DELAY);

	  HAL_Delay(1000);

	  HAL_DMA_Start(&hdma_memtomem_dma2_channel1, (uint32_t)&led_state[1], (uint32_t)&GPIOB->ODR, 1);
	  HAL_DMA_PollForTransfer(&hdma_memtomem_dma2_channel1, HAL_DMA_FULL_TRANSFER, HAL_MAX_DELAY);
